...
for i in range(1, len(df)):
    ...
    if bull:
        if low[i] < psar[i]: bull, psar[i], ep, af = False, ep, low[i], 0.02
        else:
            if high[i] > ep: ep, af = high[i], min(af + 0.02, 0.2)
            psar[i] = min(psar[i], high[i-1], high[i-2] if i > 1 else high[i-1])
    else:
        if high[i] > psar[i]: bull, psar[i], ep, af = True, ep, high[i], 0.02
        else:
            if low[i] < ep: ep, af = low[i], min(af + 0.02, 0.2)
            psar[i] = max(psar[i], low[i-1], low[i-2] if i > 1 else low[i-1])
if (is_uptrend and close > psar[-1]) or (not is_uptrend and close < psar[-1]): score += 1

direction = "📈 شراء (BUY)" if is_uptrend else "📉 بيع (SELL)"
tp, sl = (close * 1.01, close * 0.995) if is_uptrend else (close * 0.99, close * 1.005)
return {"direction": direction, "entry": round(close, 5), "tp": round(tp, 5), "sl": round(sl, 5)}, score
except Exception as e:
print(f"خطأ داخلي في حساب المؤشرات: {e}")
        return None, 0
